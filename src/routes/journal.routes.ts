...
import { protect } from '../middlewares/auth.middleware'; // Assume this path is correct
import Journal, { IJournalLine } from '../models/Journal.model';
...
// Extend the Request object (based on your existing pattern)
...
async function generateVoucherNumber(companyId: string | Types.ObjectId): Promise<string> {
...
router.post('/', protect, async (req: AuthRequest, res: Response) => {
    ...
    const { 
        companyId, date, voucherType, narration, lines, totalDebit, totalCredit 
    } = req.body;
    ...
    try {
        const ledgerIds = lines.map((line: IJournalLine) => line.ledgerId);
        const foundLedgers = await Ledger.find({ 
            _id: { $in: ledgerIds },
            companyId: new Types.ObjectId(companyId) // Ensure ledgers belong to the company
        });
        ...
        const newJournal = new Journal({
            ...
            lines: lines.map((line: any) => ({
                ...line,
                ledgerId: new Types.ObjectId(line.ledgerId)
            }))
        });
...
// =================================================================
router.get('/ledgers/:companyId', protect, async (req: AuthRequest, res: Response) => {
    const { companyId } = req.params;
    const userId = req.user?.id;

    if (!userId || !Types.ObjectId.isValid(companyId)) {
...
router.get('/', protect, async (req: AuthRequest, res: Response) => { 
    ...
    // 1. Validation
    if (!userId || !companyId || !Types.ObjectId.isValid(companyId)) {
...
// =================================================================
router.get('/report/ledger', protect, async (req: AuthRequest, res: Response) => {
    const { companyId, ledgerId, startDate, endDate } = req.query;
    const userId = req.user?.id;

    // 1. Validation (remains the same)
    if (!userId || !Types.ObjectId.isValid(companyId as string) || !Types.ObjectId.isValid(ledgerId as string)) {
    ...
    const ledgerObjectId = new Types.ObjectId(ledgerId as string);
    ...
    try {
        ...
        const reportData = await Journal.aggregate(pipeline);
